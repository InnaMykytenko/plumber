...
OEL;
    ...
    public function testEncryptedMessage()
    {
        ...
        if (!openssl_pkcs7_decrypt($messageStream->getPath(), $decryptedMessageStream->getPath(), 'files://'.$this->samplesDir.'smime/encrypt.crt', array('files://'.$this->samplesDir.'smime/encrypt.key', 'swift'))) {
    ...
    public function testEncryptedMessageWithMultipleCerts()
    {
        ...
        if (!openssl_pkcs7_decrypt($messageStream->getPath(), $decryptedMessageStream->getPath(), 'files://'.$this->samplesDir.'smime/encrypt2.crt', array('files://'.$this->samplesDir.'smime/encrypt2.key', 'swift'))) {
            $this->fail(sprintf('Decrypt of the message failed. Internal error "%s".', openssl_error_string()));
        }
...
OEL;
        ...
        if (!openssl_pkcs7_decrypt($messageStreamClean->getPath(), $decryptedMessageStream->getPath(), 'files://'.$this->samplesDir.'smime/encrypt.crt', array('files://'.$this->samplesDir.'smime/encrypt.key', 'swift'))) {
            $this->fail(sprintf('Decrypt of the message failed. Internal error "%s".', openssl_error_string()));
        }
