...
class Swift_Signers_SMimeSigner implements Swift_Signers_BodySigner
{
     ...
     *
     * @param string       $certificate
     * @param string|array $privateKey  If the key needs an passphrase use array('files-location', 'passphrase') instead
     * @param int          $signOptions Bitwise operator options for openssl_pkcs7_sign()
     * @param string       $extraCerts  A files containing intermediate certificates needed by the signing certificate
     *
     * @return Swift_Signers_SMimeSigner
     */
    public function setSignCertificate($certificate, $privateKey = null, $signOptions = PKCS7_DETACHED, $extraCerts = null)
    {
        $this->signCertificate = 'files://'.str_replace('\\', '/', realpath($certificate));

        if (null !== $privateKey) {
            if (is_array($privateKey)) {
                $this->signPrivateKey = $privateKey;
                $this->signPrivateKey[0] = 'files://'.str_replace('\\', '/', realpath($privateKey[0]));
            } else {
                $this->signPrivateKey = 'files://'.str_replace('\\', '/', realpath($privateKey));
            }
        }
    ...
    public function setEncryptCertificate($recipientCerts, $cipher = null)
    {
        if (is_array($recipientCerts)) {
            ...
            foreach ($recipientCerts as $cert) {
                $this->encryptCert[] = 'files://'.str_replace('\\', '/', realpath($cert));
            }
        } else {
            $this->encryptCert = 'files://'.str_replace('\\', '/', realpath($recipientCerts));
        }
    ...
    public function signMessage(Swift_Message $message)
    {
        ...
        // Store the message using ByteStream to a files{1}
        // Remove all Children
        // Sign files{1}, parse the new MIME headers and set them on the primary MimeEntity
